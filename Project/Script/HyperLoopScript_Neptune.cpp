...
HyperLoopScript_Neptune::HyperLoopScript_Neptune()
	: CScript((UINT)SCRIPT_TYPE::HYPERLOOPSCRIPT_NEPTUNE)
	, mTime(0.f)
	, mSetTime(100.f)
	, mSpeed(50000.f)
	, DeathCount(0)
...
void HyperLoopScript_Neptune::BeginOverlap(CCollider2D* _Other)
{
...
void HyperLoopScript_Neptune::MonsterSpawn()
{
	...
	SpawnGameObject(Monster, Vec3(250000.f, 0.f, 8995000.f), L"Monster");
}

void HyperLoopScript_Neptune::HyperLoopStateBox()
{
	...
	light->MeshRender()->SetMaterial(CResMgr::GetInst()->FindRes<CMaterial>(L"HyperLoopStateMtrl_Neptune"), 0);
	...
	SpawnGameObject(light, Vec3(250007.f, -2556.f, 8999330.f), L"HL_Light");
}
