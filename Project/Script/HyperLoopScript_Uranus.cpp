...
HyperLoopScript_Uranus::HyperLoopScript_Uranus()
	: CScript((UINT)SCRIPT_TYPE::HYPERLOOPSCRIPT_URANUS)
	, mTime(0.f)
	, mSetTime(100.f)
	, mSpeed(50000.f)
	, DeathCount(0)
...
void HyperLoopScript_Uranus::tick()
{
	...
	if (Active) //하이퍼루프 기능 활성화
	{
		...
		if (collision) //플레이어와 하이퍼루프 충돌 상태 확인
		{
			...
			{
				collision = false;
				HyperLoop_Neptune_MonsterSpawn = true;
				mTime = 0.f;
			}
...
void HyperLoopScript_Uranus::BeginOverlap(CCollider2D* _Other)
{
...
void HyperLoopScript_Uranus::MonsterSpawn()
{
	...
	SpawnGameObject(Monster, Vec3(250000.f, 0.f, 7995000.f), L"Monster");
}

void HyperLoopScript_Uranus::HyperLoopStateBox()
{
	...
	light->MeshRender()->SetMaterial(CResMgr::GetInst()->FindRes<CMaterial>(L"HyperLoopStateShaderMtrl_Uranus"), 0);
	...
	SpawnGameObject(light, Vec3(250007.f, -2556.f, 7999330.f), L"HL_Light");
}
